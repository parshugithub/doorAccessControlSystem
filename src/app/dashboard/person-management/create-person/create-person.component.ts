...
addTag(){
  ...
  constructor(public userService: UserService,private router: Router,private route: ActivatedRoute,public datepipe: DatePipe,private toaster:ToastrService,
    public personService: PersonService,public vcRef: ViewContainerRef, private cpService: ColorPickerService) { }
...
this.personService.getPersonClass(this.data.id).subscribe((res: PersonClass[])=>{
  this.personclassArray=res;
  console.log("we are using person class list",this.personclassArray);
})
this.personService.getSchedueles(data.id,this.siteid).subscribe((response: ScheduleMain[]) => {
...
getFlat(){
  ...
  this.personService.gettingFlatNumber(data.id,this.siteid).subscribe((channelResponse: Property[])=>{
...
// this.finalflat=this.properties;
//  this.person.property=this.finalflat;
 console.log("cvdcgdvcgdvcd",this.person)
    this.personService.create(this.data.id,this.siteid,this.person).subscribe(
      (res) => {
