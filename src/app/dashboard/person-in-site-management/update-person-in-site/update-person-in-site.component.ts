...
@Component({
  selector: 'app-update-person-in-site',
  templateUrl: './update-person-in-site.component.html',
  styleUrls: ['./update-person-in-site.component.scss']
})
export class UpdatePersonInSiteComponent implements OnInit {
  siteSelect:any;
...
persongroupObj = new PersonGroup();
  ...
  constructor(public StaffManagementservice: StaffService,public userService: UserService,private personservice: PersonService, private router: Router,private doorGroupService:DoorGroupService,
    ...
    private toaster: ToastrService, public personInSiteService: PersonInSiteService, private siteManagementservice: SitemanagementService,
  ...
  gettingAllPersons() {
    let data = JSON.parse(localStorage.getItem("acesscontroldata"));
    this.personInSiteService.gettingAllPesonsInSite(data.id).subscribe((data: Person[]) => {
      console.log(data);
      this.personarray = data;
      console.log("all persons are", this.personarray);
      for(let i=0;i<this.personarray.length;i++){
        console.log("this is for person array",this.personarray[i])
  ...
  gettingAllDoorGroups(){
    let data = JSON.parse(localStorage.getItem("acesscontroldata"));
console.log("this is for sitescwcew id",this.personUpdateWithAll.siteid)    
    this.doorGroupService.getAll(this.personUpdateWithAll.siteid,data.id).subscribe((res: DoorGroup[])=>{
...
getPersonGroups(){
  let data = JSON.parse(localStorage.getItem("acesscontroldata"));
console.log("this is for sits id",this.personUpdateWithAll.siteid)
  this.personInSiteService.getAllPersongroupInForSite(data.id,this.personUpdateWithAll.siteid).subscribe((response: PersonGroup[]) => {
    console.log("the staff group ", response)
    ...
    for(let i=0;i<this.persongroup.length;i++){
      console.log("id is ",this.persongroup[i].pgroupid)
  ...
  onChangeperson(value) {
    ...
    this.personInSiteService.getOnlyPersonInSite(this.personid).subscribe(personresponse => {
      this.selectedPersonGroups.length = 0;
      this.selectedDoorGroup.length = 0;
      this.selectedDoors.length = 0;
     // this.selectedSites.length = 0;

      // for (let i = 1; i < 8; i++) {numberOfPersonGroups
   ...
   console.log("this is for",this.personUpdateWithAll)
   this.gettingAllDoorGroups();
   this.getPersonGroups();
   let data = JSON.parse(localStorage.getItem("acesscontroldata"));

console.log("this is for stes id",this.personSelectedSite);
   for (let i = 0; i < this.personUpdateWithAll.persongroup.length; i++) {
    console.log("staff id is ", this.personUpdateWithAll.persongroup[i].pgroupid)
    this.val = this.personUpdateWithAll.persongroup[i].pgroupid;
...
console.log("this is for person id",this.personid);
console.log("this is for cuatomer id",this.data.id);
console.log("the complete respose is ",this.personUpdateWithAll.siteid);

    console.log("total Object is ",this.personUpdateWithAll)

  
    this.personInSiteService.updatePersonInSiteWithAll(this.personid, this.data.id,this.personUpdateWithAll.siteid,this.personUpdateWithAll).subscribe(
      (res) => {
